...
from .field_tester import (
    activate_mat_input_field,
    find_field_element,
    submit_form,
)
...
def blind_xss_injection(
    campos_validos, page, browser, url_ouvinte, url_original, playwright_instance
):
        ...
        for campo in campos_validos:
            ...
            for payload_type in payload_types:  # Um payload de cada tipo por campo
                try:
                    ...
                    print(
                        f"Recarregando página para payload {payload_type} no campo"
                        f"{field_name}"
                    )
                    page, browser = page_reload(
                        page, browser, url_original, playwright_instance
                    )
                    if not page:
                    ...
                    print(
                        f"Payload {payload_id} ({payload_type}) injetado no campo"
                        f"{field_name}"
                    )
